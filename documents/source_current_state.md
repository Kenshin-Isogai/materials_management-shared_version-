...
- `frontend/`
  ...
  - `src/components/ComboInput.tsx`: shared lightweight combobox (Radix Popover-based) — free text input with filterable dropdown list, keyboard navigation, and combobox/listbox ARIA semantics, used in Bulk Item Entry and Item List inline editing
...
- Orders manual CSV import and quotation maintenance now use external document URLs as the primary contract.
  ...
  - the import preview raw-input cell now emphasizes both `item_number` and `quantity`, so operators can verify SKU and count before checking the rest of the row metadata
