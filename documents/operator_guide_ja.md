...
> ⚠️ **注意:** 
> 見積書, 発注書のURLは暫定的に　見積番号, 注文番号 と同一の文字列にしてください。例: `order_date` = 115-112 = `purchase_order_document_url`
...
2. **Date** フィールドに分析したい基準日（開始予定日）を入力
   保存されている開始予定日が今日より前の場合、Planning Board はその値を自動更新せず、分析日だけ当日（JST）に補正して表示します
