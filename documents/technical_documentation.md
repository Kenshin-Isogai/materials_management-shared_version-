...
- Requirement entry includes a preview-first bulk text parser (`item_number,quantity` per line).
  ...
  - `POST /api/projects/requirements/preview/unresolved-items.csv` exports unresolved preview rows as an Items import-compatible CSV with default `row_type=item`, `manufacturer_name=UNKNOWN`, and `units_per_order=1`; the Projects UI now sends the reviewed preview snapshot so export stays aligned with the visible preview even if the textarea changes later
