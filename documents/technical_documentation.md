...
- Frontend user administration at `/users` page.
  ...
  - the `/registration` holding flow now periodically re-checks approval state for signed-in unmapped identities, so admin approval in another browser/session can promote the applicant without forcing a manual sign-out/sign-in cycle
