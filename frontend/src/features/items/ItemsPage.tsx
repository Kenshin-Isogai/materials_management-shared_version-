...
import type { CatalogSearchResult, Item, Manufacturer, Supplier } from "@/lib/types";
import type {
  ItemEntryRow,
  ItemImportPreview,
...
export function ItemsPage() {
  ...
  const manufacturerOptions = manufacturersData?.map((row) => row.name) ?? [];
  const supplierOptions = suppliersData?.map((row) => row.name) ?? [];
